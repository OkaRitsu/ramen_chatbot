...
def search_similar_stores(virtual_store, user_area):
    avg_scores = []
    # 実在する店舗
    ...
    #地区指定があればsearch_area()で絞る
    if user_area != "":
        ranking = search_area(user_area, ranking)
        return ranking
    ...
    return recommend_stores

print(search_similar_stores(['味噌ラーメン', '二郎', 'コーン', 'もやし'], "足立区").loc[:,["name"]])